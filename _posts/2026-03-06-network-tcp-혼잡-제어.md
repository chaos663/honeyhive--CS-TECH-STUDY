...
## Deep Dive: Reno → Cubic → BBR 진화

### 알고리즘 복잡도

TCP 혼잡 제어 알고리즘은 각 이벤트(ACK 수신, 패킷 손실)마다 `cwnd`와 `ssthresh`를 업데이트합니다. 모든 표준 알고리즘에서 **이벤트당 O(1) 시간 복잡도**, **O(1) 공간 복잡도**로 동작하여 네트워크 스택의 오버헤드를 최소화합니다.

### 손실 기반 vs 측정 기반

전통적인 Reno/Cubic은 **패킷 손실을 혼잡의 신호**로 사용합니다. 이 방식은 버퍼가 가득 찰 때까지 대역폭을 높이므로, 불필요한 지연(Bufferbloat)이 발생합니다.

BBR은 **병목 대역폭(BtlBw)**과 **최소 RTT(RTprop)**를 직접 측정하여 `cwnd`를 결정합니다. 패킷 손실 없이도 최적의 전송 속도를 찾아, 높은 처리량과 낮은 지연을 동시에 달성합니다.
