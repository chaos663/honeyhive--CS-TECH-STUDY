...
**Q1: 배열과 연결 리스트의 핵심 차이점은?**
A1: 배열은 연속 메모리에 저장되어 인덱스 접근이 O(1)이지만 삽입/삭제가 O(n)입니다. 연결 리스트는 포인터로 연결되어 삽입/삭제가 O(1)(위치를 알 때)이지만 접근이 O(n)입니다. 캐시 지역성(Cache Locality)도 배열이 유리합니다.

**Q2: 동적 배열의 크기 확장은 어떻게 동작하나요?**
A2: 용량이 가득 차면 보통 2배 크기의 새 배열을 할당하고 기존 데이터를 복사합니다. 개별 확장은 O(n)이지만, 분할 상환 분석(Amortized Analysis)으로 평균 O(1)입니다.

**Q3: Java의 ArrayList와 LinkedList 중 어떤 것을 선택해야 하나요?**
A3: 대부분의 경우 ArrayList가 유리합니다. 캐시 지역성이 좋고, 인덱스 접근이 빈번한 상황에서 압도적으로 빠릅니다. LinkedList는 리스트 앞쪽에 빈번한 삽입/삭제가 필요하고, 인덱스 접근이 거의 없는 경우에만 유리합니다.

**Q4: 연결 리스트에서 중간 노드를 찾는 효율적인 방법은?**
A4: **러너(Runner) 기법** — slow 포인터는 한 칸씩, fast 포인터는 두 칸씩 이동합니다. fast가 끝에 도달하면 slow가 중간에 위치합니다. O(n) 시간, O(1) 공간입니다.

**Q5: 배열 기반 자료구조에서 캐시 지역성이 중요한 이유는?**
A5: 배열은 메모리가 연속적이어서 CPU 캐시 라인에 여러 요소가 함께 로드됩니다. 연결 리스트는 노드가 메모리에 흩어져 있어 캐시 미스가 빈번하게 발생하므로, 동일한 O(n) 순회라도 배열이 실제로 훨씬 빠릅니다.
